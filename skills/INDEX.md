...
| **cogito-teacher** | running a structured teaching session for a specific learner — the golden structure (Profile / Loop / Progress / Content), presentation- and learner-agnostic. Composes adaptive-learning + cogito-review + lesson objects; adds the constrained tutor (content-LOCK, hint-ladder, mastery gate). Runnable: `scripts/cogito-teach.sh`. | machinery verified live (driver surfaces the due rep + next unlocked lesson); the teaching *loop* awaits its first live lesson — on branch |
