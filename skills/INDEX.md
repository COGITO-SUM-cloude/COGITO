...
| **cogito-panelist** | inside a council — one panel voice: answers the question INDEPENDENTLY in an assigned lens (pragmatist / skeptic / first-principles / domain-expert / contrarian / research), grounds its claims, names confidence + blind spots, returns the shared panelist JSON shape. Minimal tool surface (Read/Grep/Web) so a 3–5 agent summon stays cheap. Spawn 3–5 in one message. |
