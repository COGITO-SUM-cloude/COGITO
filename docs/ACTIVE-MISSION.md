...
- **CMS deploy fix — do FIRST in the new session:** `idk-whatimdoing` live site shows the OLD plain login; the finished "Obsidian & Ember" redesign is on a `claude-cogito` branch, not merged. **Vercel builds production from `main` only** → merge that branch into `main`, then verify `idk-whatimdoing.vercel.app` from a logged-out browser.
